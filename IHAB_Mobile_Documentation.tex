...
This application is still undergoing development. Please feel free to contact us in case of any bugs or difficulties occurring during installation and usage. The given directions in the operation system settings might vary between devices.
...
\vspace{0.5cm}
	...
	\item Now that a connection has been verified, move via the console to the directory that holds the .apk file and input\\
	\\
	\colorbox{black!10}{\texttt{adb install IHAB\_Mobile\_Standalone.apk}}.\\
	\\
	This will install the application. In case a previous version of the application is already installed, type\\
	\\
	\colorbox{black!10}{\texttt{adb install-r IHAB\_Mobile\_Standalone.apk}}.\\
	
	\item In order to make the application a device owner, which is needed for it to run as a dedicated device, type\\
	\\
	\colorbox{black!10}{\texttt{adb shell dpm set-device-owner \newline com.fragtest.android.pa/.AdminReceiver}}.\\
...
\\
The menu view (see figure on the right) of the application features a bar at the top with a \textit{Help} button, providing a general explanation of the main answer formats. Next to it is a microphone symbol that can change its color according to the type of data input. On the right hand side is a battery symbol displaying the current state of charge.\\
\\
...
The \textit{IHAB} application offers 6 different modes of operation which are indicated by the color of the microphone symbol at the top center of the screen. If no objective data connection can be established (not needed in Standalone mode), the symbol is gray \includegraphics[width=0.02\textwidth]{images/mic_gray.png}.

\begin{itemize}
	\item \textbf{STANDALONE} \includegraphics[width=0.02\textwidth]{images/mic_airplaneblue.png}: Application is used for questionnaire purposes only, no additional device needed.
	\item \textbf{INTERNAL MIC} \includegraphics[width=0.02\textwidth]{images/mic_orange.png}: Objective data is extracted using the internal microphone or the microphone channel primarily selected by the smartphone.
	\item \textbf{USB} \includegraphics[width=0.02\textwidth]{images/mic_jadered.png}: Audio data is obtained from a USB audio device.
	\item \textbf{A2DP} \includegraphics[width=0.02\textwidth]{images/mic_green.png}: Bluetooth connection using the A2DP protocol. Since this type of connection requires a specialised operating system, it will only run on an LG Nexus 5.
	\item \textbf{RFCOMM} \includegraphics[width=0.02\textwidth]{images/mic_violet.png}: Bluetooth connection using the RFCOMM protocol via the IHAB first generation bluetooth device.
	\item \textbf{PHANTOM} \includegraphics[width=0.02\textwidth]{images/mic_darkblue.png}: Bluetooth connection using the RFCOMM protocol via the IHAB second generation bluetooth device.
\end{itemize}

In case of a bluetooth connection, the device must be paired prior to starting the application via the bluetooth settings of the smartphone or tablet computer. This is achieved via \textit{Settings}$\rightarrow$\textit{Connected Devices}$\rightarrow$\textit{Pair new device}.
...
Questionnaire files are placed in the \textit{quest} folder located in the application directory. The application will automatically sort all questionnaire files alphabetically and pick the uppermost. If you would like to use a different questionnaire, you can choose manually from the preferences menu (see \ref{sub:experimenter}), but we recommend placing only a single file in the \textit{quest} folder in order to avoid confusion. 
...
Checkboxes give the user the impression of a list of which multiple items can be selected at the same time. Customized within-list logic can be implemented as described in \ref{subsec:goupingandexclusivity}. 
...
Notice that entries can be spread across multiple lines in order do increase readability.
...
\begin{figure}[h]
	\centering
		\includegraphics[width=0.70\textwidth]{images/DataExtractor.png}
